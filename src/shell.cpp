...
std::string format_perms(uint16_t mode) {
    if (mode == INODE_DIR) return "drwxr-xr-x";
    if (mode == INODE_FILE) return "-rw-r--r--";
    return "----------";
}
...
void do_ls(disk &dsk, const superblock &sb, const std::vector<std::string> &args) {
    bool show_all = false;
    std::string path = "/";

    for (size_t i = 1; i < args.size(); i++) {
        if (args[i] == "-la" || args[i] == "-al" || args[i] == "-l" || args[i] == "-a") {
            show_all = true;
        } else {
            path = args[i];
        }
    ...
    for (const auto &entry : entries) {
        ...
        if (!show_all && name[0] == '.') continue; // hide hidden files unless -a

        if (show_all) {
            inode child_ino;
            if (read_inode(dsk, sb, entry.inode, child_ino)) {
                std::cout << format_perms(child_ino.mode) << " "
                          << std::setw(4) << child_ino.size << " "
                          << name << "\n";
            }
        } else {
            std::cout << name << "  ";
    ...
    if (!show_all && !entries.empty()) std::cout << "\n";
}

void do_mkdir(disk &dsk, const superblock &sb, const std::vector<std::string> &args) {
    if (args.size() < 2) {
    ...
    if (parent_ino == UINT64_MAX) {
        std::cerr << "mkdir: cannot create directory '" << path << "': No such file or directory\n";
        return;
    }

    if (dir_create(dsk, sb, parent_ino, name.c_str()) == UINT64_MAX) {
        std::cerr << "mkdir: cannot create directory '" << path << "': Failed (might already exist)\n";
    }
}

void do_touch(disk &dsk, const superblock &sb, const std::vector<std::string> &args) {
    if (args.size() < 2) {
...
void do_write(disk &dsk, const superblock &sb, const std::vector<std::string> &args) {
    if (args.size() < 3) {
    ...
    if (!file_write(dsk, sb, file_ino, 0, reinterpret_cast<const uint8_t*>(data.c_str()), data.size())) {
        std::cerr << "write: failed to write data\n";
    }
}

void do_cat(disk &dsk, const superblock &sb, const std::vector<std::string> &args) {
    if (args.size() < 2) {
...
bool recursive_rm(disk &dsk, const superblock &sb, uint64_t parent_ino, uint64_t target_ino, const std::string &name) {
    inode target;
    if (!read_inode(dsk, sb, target_ino, target)) return false;
...
void do_rm(disk &dsk, const superblock &sb, const std::vector<std::string> &args) {
    if (args.size() < 2) {
...
void do_stat(disk &dsk, const superblock &sb, const std::vector<std::string> &args) {
    if (args.size() < 2) {
    ...
    uint64_t file_ino = path_resolve(dsk, sb, path.c_str());
    if (file_ino == UINT64_MAX) {
        std::cerr << "stat: cannot stat '" << path << "': No such file or directory\n";
        return;
    }
    ...
    if (read_inode(dsk, sb, file_ino, ino)) {
        std::cout << "  File: " << path << "\n"
                  << "  Size: " << ino.size << "\tBlocks: " << (ino.size > 0 ? 1 : 0) << "\n"
